...
export const load: PageServerLoad = async () => {
	...
	// read from db
	const { data, error } = await supabase.from('files').select().eq('team', 6).order('date', { ascending: false });
	const { data: dataEvent, error:errorEvent } = await supabase.from('event').select().eq('team', 6).order('id', { ascending: true });
